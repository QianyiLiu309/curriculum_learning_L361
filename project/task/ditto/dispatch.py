...
from project.task.ditto.train_test import get_fed_eval_fn, test, train
...
def dispatch_train(
    ...
    if train_structure is not None and train_structure.upper() == "DITTO":
...
def dispatch_data(cfg: DictConfig) -> DataStructure | None:
    ...
    if client_model_and_data is not None and partition_dir is not None:
        ...
        if client_model_and_data.upper() == "DITTO_CNN":
            return (
                get_cnn,
                client_dataloader_gen,
                fed_dataloader_gen,
