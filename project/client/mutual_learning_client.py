...
class MutualLearningClient(fl.client.NumPyClient):
    ...
    def fit(
        ...
        # load parameters from federated model
    ...
    def load_local_parameters(self, config: dict, checkpoint_path: str) -> nn.Module:
        """Load local parameters from disk."""
        net = self.net_generator(config, self.rng_tuple)
        if Path(checkpoint_path).exists():
            net.load_state_dict(torch.load(checkpoint_path))
            print(f"Loaded local parameters from {checkpoint_path}")
        return net
...
def get_client_generator(
    ...
    def client_generator(cid: CID) -> MutualLearningClient:
        """Return a new Client.
